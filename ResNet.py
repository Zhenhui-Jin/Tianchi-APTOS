...
def get_backbone_ResNet101():
    backbone = ResNet101(include_top=False, input_shape=[None, None, 3])
    outputs = [
        backbone.get_layer(layer_name).output
        for layer_name in ['conv2_block3_out', 'conv3_block4_out', 'conv4_block23_out', 'conv5_block3_out']
        ]
...
class FPN(Layer):
    ...
    def __init__(self, backbone=None, **kwargs):
        ...
        self.conv_c2_1x1 = Conv2D(filters=256, kernel_size=1, strides=1, padding='same')
        ...
        self.conv_c2_3x3 = Conv2D(filters=256, kernel_size=3, strides=1, padding='same')
        ...
        self.sampling_2x = UpSampling2D(2)

    def call(self, inputs, training=False, *args, **kwargs):
        c2_output, c3_output, c4_output, c5_output = self.backbone(inputs, training=training)
        p2_output = self.conv_c2_1x1(c2_output)
        p3_output = self.conv_c3_1x1(c3_output)
        p4_output = self.conv_c4_1x1(c4_output)
        p5_output = self.conv_c5_1x1(c5_output)

        p4_output = p4_output + self.sampling_2x(p5_output)
        ...
        p6_output = p5_output + self.upsample_2x(p5_output)
...
def build_head(output_filters, bias_init):
    ...
    for _ in range(4):
        head.add(Conv2D(filters=256, kernel_size=3, padding='same', kernel_initializer=kernel_init))
        head.add(ReLU())
    head.add(Conv2D(output_filters, kernel_size=3, padding='same', kernel_initializer=kernel_init,
...
if __name__ == '__main__':
    input_shape = (3, 500, 764)
    X_input = keras.Input(shape=input_shape)
    X = tf.zeros([1, 3, 500, 764])
