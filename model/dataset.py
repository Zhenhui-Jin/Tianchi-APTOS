...
class CSVDataset(Dataset):
    def __init__(self, config: CSVConfig, data: pd.DataFrame):
        """
        数据加载器
        """
        self.data = data.copy()
    ...
    def __getitem__(self, index):
        ...
        regression_label = None
        columns = self.data.columns.tolist()
        label_regression = self.config.label_regression
        if label_regression and label_regression in columns:
            regression_label = data[[label_regression]].array
            regression_label = torch.Tensor(regression_label).float()

        classify_label = None
        label_names = []
        label_classify = self.config.label_classify
        for name in label_classify:
            if name in columns:
                label_names.append(name)

        if label_classify and label_names:
            classify_label = data[label_classify].array
            classify_label = torch.Tensor(classify_label).float()

        features = torch.Tensor(data[self.config.feature_columns].array).float()
